...
3. В браузере открывается `http://127.0.0.1:8080/dashboard`, появляется окно **API-ключ**, вы вставляете ключ и видите панель (цифры могут быть нулями — это нормально).
...
1. Должно появиться окно **Введите ADMIN_API_KEY**. Если его нет — нажмите кнопку **API-ключ** справа вверху.
2. В Termux выполните `bash termux/show-key-termux.sh` и скопируйте значение после `ADMIN_API_KEY=`.
3. Вставьте ключ в окно панели и нажмите **Сохранить**.

Ключ запоминается в браузере этого телефона. После этого панель показывает статистику (сначала нули) и таблицу лидов.

Если видите зелёную полосу «ОЖИДАНИЕ ЗАПУСКА JS…» и кнопка не реагирует — это старая сломанная версия скрипта. Обновите код (шаг 3), затем:
...
| Кнопка **API-ключ** молчит | Старый `dashboard.js`. Обновите код и `bash termux/sync-termux.sh`, затем новая вкладка |
