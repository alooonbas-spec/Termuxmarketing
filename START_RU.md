...
3. В браузере открывается `http://127.0.0.1:8080/dashboard`, справа видно окно для ключа и кнопку **API-ключ**, вы вставляете ключ, жмёте кнопку и видите панель (цифры могут быть нулями — это нормально).
...
1. Справа вверху сразу видно окно для ключа и рядом кнопку **API-ключ**.
2. В Termux выполните `bash termux/show-key-termux.sh` и скопируйте значение после `ADMIN_API_KEY=`.
3. Вставьте ключ в окно и нажмите **API-ключ**.
